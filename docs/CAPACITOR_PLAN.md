...
npx cap init "RadioScout" "gq.danread.radioscout" --web-dir dist/client
npx cap add android
...
1. ~~App ID + display name~~ → **"Scout - Your Home, Mapped"** / `gq.danread.scout` (matches `scout.danread.gq`). Open micro-decision: launcher label short `"Scout"` vs full title.
