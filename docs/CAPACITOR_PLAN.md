...
- [ ] M9 (re-scoped 2026-09-20): OTA updates for sideloaded APKs run on zero services — version manifest lives in-repo (`public/ota/<channel>.json`, served by Pages), bundles ride GitHub Release assets (`ota-<channel>-<version>` tags, kept separate from the `v*` APK tags), version pick is client-side (`src/lib/ota.ts`), publish via `pnpm ota:publish` (builds/zips/uploads, keeps last 3 per channel, then you commit + push the manifest). First publish still yours (needs a public repo + `gh auth login`). Native-layer edits (manifest, plugins, Capacitor itself) still need a full APK regardless. `[2026-09-21]`: version basis fixed — the pick compares against the live bundle version (`CapacitorUpdater.current()`, `NativeShell.tsx`), not the immutable native versionName, so a staged bundle is never re-picked every boot. Freshly staged bundles surface a restart toast (`reload()` on tap); APK releases surface separately via `AppUpdateDialog.tsx` (daily `releases/latest` check vs installed versionName, Download opens the APK asset in the system browser via `@capacitor/browser`). Verifying end to end needs a device: publish to a scratch channel, point a debug build at it with `VITE_OTA_URL`, install, confirm stage → restart toast → new bundle active (`current()` id changes) and no re-stage on the next boot; then delete the scratch release. Semi-automated via Actions → Publish OTA (dispatch: guarded on the matching `v*` APK release, checksum-verifies the manifest URL, commits the manifest back).
