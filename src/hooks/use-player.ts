...
import { isHlsUrl, pickPlayableUrl, type Station } from "@/lib/radio/types";
  ...
  if (!audio) {
    ...
    audio.addEventListener("error", () => {
      if (snapshot.status === "loading") {
        emit({ status: "error", error: "This stream wouldn't play. It may be offline or an unsupported format." });
      }
    });
  }
  return audio;
...
export function play(station: Station): void {
  ...
  void (async () => {
    ...
    try {
      ...
      if (name !== "AbortError") {
        emit({ status: "error", error: "Couldn't start playback. Check your connection and try again." });
      }
    }
  })();
}
