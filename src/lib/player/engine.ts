...
/**
 * Whether a station's host can be routed through Web Audio. Predicate lives
 * in `lib/player/leveling` — this wrapper supplies the session flags.
 */
function canRouteStation(station: Station | null): boolean {
  return canRouteLeveling(station, normalizeOn, blockedHosts);
}
 ...
 * host already proved unanalysable this session. Run construction lives in
 * `lib/player/leveling` — the flags stay here.
 */
function maybeRouteAudio(element: HTMLAudioElement): void {
  audioRouted = false;
  retriedDirect = false;
  const run = routeLevelingAudio(element, snapshot.station, normalizeOn, blockedHosts);
  if (!run) {
    audioRouted = false;
    return;
  }
...
function ensureLiveContext(): void {
  resumeLevelingContext(audioCtx);
}
...
/**
 * Sleep-timer deadline lives in `lib/player/sleep-timer` (owns the timeout
 * plus both native/web arms and `sleepEndsAt`). This module only supplies
 * the audible continuation: no-op unless playing, else fade out and pause
 * (`fadeOutAndPause` lives alongside transport — one state machine).
 */
 ...
 * touch (see `applyPlayerPrefs`) bumps it to abort the ramp. Stepping lives
 * in `lib/player/fades` — this wrapper only owns the token.
 */
function fadeRamp(from: number, to: number, ms: number, onDone?: () => void): void {
  const token = ++fadeToken;
  runFadeRamp(from, to, ms, setFadeLevel, () => token !== fadeToken, onDone);
}
...
function handoffToIncoming(station: Station, handoff: number): void {
  ...
  rampElement(next, 0, userLevel(), CROSSFADE_MS, () => handoff !== handoffToken);
}
